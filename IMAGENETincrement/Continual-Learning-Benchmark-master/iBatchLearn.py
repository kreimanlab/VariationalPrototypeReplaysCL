...
def run(args):
    ...
    # Prepare dataloaders
    train_dataset, val_dataset = dataloaders.base.__dict__[args.dataset](args.num_train_sams, args.dataroot, args.train_aug)
    print('Loading Dataset with size: ' + str(len(train_dataset)))
    ...
    if args.offline_training:  # Non-incremental learning / offline_training / measure the upper-bound performance
        ...
        for i in range(len(task_names)):        
            ...
            #print('train_name')
            #print(train_name)
            test_dataset = val_dataset_splits[train_name]
            test_dataset_all.append(test_dataset)
        #print('test_dataset_all')
        #print(test_dataset_all)
        #print('test_dataset_all subsets:')
        #print(test_dataset_all[:3])
        
        # Feed data to agent and evaluate agent's performance
        for i in range(len(task_names)):
            train_name = task_names[i]
            print('======================',train_name,'=======================')
            print('dataset size: ' + str(len(train_dataset_splits[train_name])))
...
#                for q, (input, target, task) in enumerate(train_loader):
#                    print(str(q) + 'target = ' + str(target))
#            else:
#                print(len(train_loader))
...
#                acc_table[val_name][train_name] = agent.validation(val_loader)
            ...
            print('validation split name up to:', val_name)
            val_data = torch.utils.data.ConcatDataset(test_dataset_all[:i+1]) if not args.eval_on_train_set else train_dataset_splits[val_name]
            #print(val_data)
            val_loader = torch.utils.data.DataLoader(val_data,
                                                     batch_size=args.batch_size, shuffle=False,
                                                     num_workers=args.workers)
            acc_table[val_name] = agent.validation(val_loader)
...
def get_args(argv):
    ...
    parser.add_argument('--model_type', type=str, default='lenet', help="The type (mlp|lenet|vgg|resnet) of backbone network")
    parser.add_argument('--model_name', type=str, default='LeNetC', help="The name of actual model for the backbone")
    ...
    parser.add_argument('--num_train_sams', type=int, default = 1000, help="The number of training samples")
...
if __name__ == '__main__':
    ...
    for reg_coef in reg_coef_list:
        ...
        for r in range(args.repeat):
            ...
            path = os.path.join('outputs/split_IMAGENET_incre_class/', '{firstname}_{secondname}-precision_record.pt'.format(firstname=args.agent_name,secondname=(r+1)))
            torch.save(acc_table, path)
            
            print('accuracy table for repeat')
            print(acc_table)
