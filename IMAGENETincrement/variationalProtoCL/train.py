...
def train(model, train_datasets, test_datasets, task_output_space, args, cuda=False):
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        if task == 1:
            ...
            args.dataset_samples = min(args.oriproto_eachsize, args.recall_eachsize)*2
            args.dataset_episodes = int(math.floor(args.dataset_total2/args.dataset_samples))
        ...
        if task > 1:
            ...
            if args.epochs_per_tasknext < 5:
                args.epochs_per_tasknext = 5
        ...
        for epoch in range(1, args.epochs_per_task+1):
            ...
            for batch_index, (imgs, labels, dummy) in data_stream:
                ...
                for t in range(args.dataset_classes-args.dataset_current_classes):
                    ...
                    if args.dataset_samples != args.oriproto_eachsize:
                        rn = np.random.permutation(args.oriproto_eachsize)
                        rn = rn[:args.dataset_samples/2]
                        samplesingleimgs = oriprotos[t][rn,:].view(1,args.dataset_samples/2,args.dataset_channels, args.dataset_width, args.dataset_width)
                    #size: num_class, num_sample = 1, num_channle, num_width, num_width
                    else:
                        samplesingleimgs = oriprotos[t][:,:].view(1,args.dataset_samples/2,args.dataset_channels, args.dataset_width, args.dataset_width)
                ...
                if iternum%args.replay_freq == 0:
                    ...
                    for t in range(task-2):
                        ...
                        for T in range(task-2):#range(task-2,-1,-1): #range(task-1) in asceding order; range(task-2,-1,-1) in descending order; range(task-2,task-3,-1) in prev task only
                            ...
                            subsetoldimgs = torch.Tensor()
                            for t in range(numclass_all[T]):
                                
                                #generate a random index for selecting ori protos for one image per class
                                rn = random.randint(0,args.oriproto_eachsize-1)
                                samplesingleimgs = oriprotos[t][rn,:].view(1,1,args.dataset_channels, args.dataset_width, args.dataset_width)
                                if cuda: samplesingleimgs = samplesingleimgs.cuda()
                                
                                if subsetoldimgs.nelement() == 0:
                                    subsetoldimgs = samplesingleimgs
                                else:
                                    subsetoldimgs = torch.cat((subsetoldimgs,samplesingleimgs),0)
                            #print('subsetoldimgs')
                            #print(subsetoldimgs.size())#print('oldimgs')
                            #print('numclass_all[t]')
                            #print(numclass_all[T])
                            subsetoldimgs = subsetoldimgs.view(numclass_all[T],1,args.dataset_channels, args.dataset_width, args.dataset_width)
