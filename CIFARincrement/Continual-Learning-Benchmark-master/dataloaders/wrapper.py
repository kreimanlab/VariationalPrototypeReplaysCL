...
class CacheClassLabel(data.Dataset):
    ...
    """
    def __init__(self, dataset, number_classes):
        ...
        for i in range(len(dataset)):
            ...
            #print(target)
            labels[i] = target
        mylabels = torch.from_numpy(labels).type(torch.LongTensor)
        self.labels = torch.LongTensor(mylabels)  # It will be used as tensor in loader generators.
...
class Subclass2(data.Dataset):
    ...
    def __init__(self, dataset, class_list, remap=True):
        ...
        self.indices = []
        for c in class_list:
            self.indices.extend((dataset.labels==c).nonzero().flatten().tolist()[0:10])
        if remap:
            self.class_mapping = {c: i for i, c in enumerate(class_list)}
